...
Навыки нужны не как пассивные проценты ради прокачки, а как способ разделить игровые роли,
инструменты и ответственность. В SS14-формате это особенно важно: игрок должен понимать, почему
кузнец нужен кузнице, травник — лекарствам, а боец — оружию, но при этом система не должна
превращаться в MMO-дерево с обязательным гриндом.

Текущее состояние в коде: навыки уже имеют прототипы, прогресс от `0` до `1`, книги обучения и
проверки доступа к отдельным взаимодействиям. Полноценного дерева зависимостей пока нет: в
прототипах не заполнены `RequiredSkills`, поэтому все реализованные навыки технически лежат на одном
уровне.
...
Это не текущее состояние кода, а предлагаемая структура для GDD и будущих задач. Она нужна, чтобы
понять, какие навыки являются базовыми, какие — специализациями, а какие завязаны на несколько веток.
...
Общие навыки не должны становиться обязательным налогом на каждое действие. Их стоит вводить только
там, где они создают игру между людьми.
...
Грамотность — самый важный кандидат на добавление в общую ветку, потому что она связывает языки,
книги, законы, экономику и роли власти.

### 4.2. Ремесленные навыки

Ремесленные навыки должны открывать рабочие места и качество результата, а не просто выдавать
абстрактный бонус. Игрок без навыка может помогать, носить материалы и выполнять простые действия,
но не должен полностью заменять мастера.
...
Боевые навыки должны усиливать владение типом оружия, но не делать новичка полностью бесполезным.
Сейчас логика уже близка к этому: прогресс влияет на бонус урона, а обучение идет через практику.
...
В MVP зависимости лучше делать мягкими. Если навык полностью закрывает слишком много действий,
игроки без него просто выпадают из игры.
