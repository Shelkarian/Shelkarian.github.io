...
4. Стартовая прочность стены равна **5 стандартным ударам кирки**. На каждом пороге в 20% потерянной прочности рядом со стеной появляется один переносимый кусок камня и слой мелких обломков.
...
8. Куски камня можно вынести вручную, а мелкие обломки — убрать лопатой.
9. Если стена содержала руду, дополнительно появляются **2–4 куска руды по `0.25u`** каждый.
...
3. После одного завершенного действия длительностью **2 секунды** в лопате появляется один кусок грязи, соответствующий породе.
...
2. Кликает ЛКМ по полу вне боевого режима.
3. Запускается действие с зеленой плашкой прогресса.
4. Длительность одного действия — **2 секунды**.
5. После каждого завершенного действия в лопате появляется один кусок грязи.
6. После **5 действий** пол опускается на **1 клетку вниз**; стартовое полное время составляет 10 секунд.
...
2. Со стартовым шансом **35%** он превращает каждый соседний твердый нерудный блок в блок с рудой.
...
3. Дополнительно появляются **2–4 куска руды**.
4. Каждый кусок содержит **`0.25u` руды**; суммарный стартовый выход стены составляет `0.50–1.00u`.
...
| **Кусок руды** | Содержит `0.25u`; из рудной стены выпадает 2–4 куска (`0.50–1.00u` суммарно). |
...
Во время раскопок под землей необходимо ставить подпорки и балки. Активная подпорка укрепляет клетки в **радиусе 2 клеток по расстоянию Чебышева**: `max(|dx|, |dy|) <= 2` на том же уровне шахты.

После выхода из укрепленной зоны счетчик `unsupportedBlocks` начинается с нуля и увеличивается на 1 за каждый раскопанный блок. Шанс обвала для очередного блока вычисляется как `collapseChance = clamp(0.01 + 0.20 * unsupportedBlocks, 0, 1)`. В укрепленной клетке шанс равен базовым `0.01`, а счетчик сбрасывается в ноль.

| Условие | Эффект |
| :--- | :--- |
| **Раскопка блока** | Базовый шанс обвала — 1%. |
| **Продолжение без балки** | Шанс обвала растет на 20 процентных пунктов за каждый блок и ограничивается диапазоном `0–100%`. |
| **Установленная подпорка/балка** | Укрепляет клетки в радиусе 2 и сбрасывает счетчик неукрепленных блоков. |
...
| **Радиус укрепления** | 2 клетки по расстоянию Чебышева на текущем уровне |
...
3. Если раскопка продолжается вне радиуса подпорки, шанс увеличивается на **20 процентных пунктов за каждый блок**: `collapseChance = clamp(0.01 + 0.20 * unsupportedBlocks, 0, 1)`.
...
`oxygen = clamp(round(randomInt(0, 100) / max(1, airDistanceCells)), 0, 100)`

`airDistanceCells` — целое расстояние в клетках до ближайшей клетки с доступом к воздуху. `max(1, airDistanceCells)` исключает деление на ноль; если доступной клетки воздуха нет, запас кислорода принимается равным `0`.
...
graph TD
    ...
    subgraph STAGE3["ЭТАП 3: РЕЗУЛЬТАТ РАСКОПКИ"]
        ...
        DigDirtFloor --> DirtCounter{5 действий выполнено?}:::check
...
| **Грунтовый пол** | Опускается | Изменение уровня клетки. | После 5 действий. |
...
| :--- | :--- | :--- | :--- |
| **Подпорка** | 4 бревна | 5 сек. на каждое бревно | Укрепляет клетки в радиусе 2 и снижает риск обвала. |
| **Балка** | 4 бревна | 20 секунд | Используется для предотвращения обвалов по правилам подпорки. |
...
| **Вертикальный грунт** | Лопата | Грунтовый пол | 2 секунды за действие, 5 действий | Пол опускается на уровень ниже. |
| **Очистка обломков** | Лопата | Заваленная/засыпанная клетка | 2 секунды за 20% покрытия | Удаление мешающих обломков. |
...
| **Количество действий для опускания грунта** | 5 действий по 2 секунды |
| **Базовый шанс обвала** | 1% |
| **Радиус подпорки/балки** | 2 клетки по расстоянию Чебышева на текущем уровне |
| **Рост шанса обвала без балки** | `clamp(1% + 20 п.п. * unsupportedBlocks, 0%, 100%)` |
...
| **Формула кислорода** | `clamp(round(randomInt(0, 100) / max(1, airDistanceCells)), 0, 100)`; без доступного воздуха — `0` |
| **Откопать для доступа воздуха** | 20% |
| **Откопать для извлечения сущности** | 80% |
| **Вместимость одного куска руды** | Не больше 1u |
| **Стартовый выход рудной стены** | 2–4 куска по `0.25u`, всего `0.50–1.00u` |
...
## Справочная библиотека руд и пород
...
| **Угольный пласт** | Матовые черные включения | Куски угля |
| **Серебряная руда** | Светлые металлические вкрапления | Куски серебряной руды |
| **Золотая руда** | Желтые металлические прожилки | Куски золотой руды |
| **Мифриловая руда** | Холодные голубые прожилки | Куски мифриловой руды |
| **Адамантитовая руда** | Темные красные кристаллы | Куски адамантитовой руды |
| **Кристалл маны** | Видимое свечение цвета месторождения | Кристаллы маны |

Новый тип руды добавляется только вместе с материалом, рудной стеной, добываемым объектом,
параметрами генерации и способом переработки.
...
- Вертикальная раскопка грунта опускает пол после 5 действий по 2 секунды и выдает один кусок грязи за действие.
- Рудная стена выдает 2–4 куска по `0.25u`, а распространение жилы проверяет соседей с шансом 35%.
- Подпорка укрепляет клетки в радиусе 2 по расстоянию Чебышева только на своем уровне.
- Шанс обвала всегда вычисляется через `clamp` и остается в диапазоне `0–100%`.
- Формула кислорода не делит на ноль; при отсутствии доступной клетки воздуха результат равен `0`.
- Каждый тип руды имеет отличимую стену, добываемый объект, параметры жилы и выход переработки.
- Справочная библиотека оформлена отдельно от последовательности игровой механики.
