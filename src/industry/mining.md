...
Главная идея — добыча не должна быть мгновенным действием через меню. Игрок физически работает с породой, инструментами, обломками, подпорками и добытыми материалами. Шахта должна ощущаться как опасная рабочая зона, где важно не только копать, но и поддерживать проходы в безопасном состоянии.
...
### Этап 2: Горизонтальная раскопка горной породы
...
> Логика из исходного файла сохранена: во время повреждения стены обломки появляются рядом со стеной, а после разрушения стены распределяются по соседним клеткам.

---

### Этап 3: Вертикальная раскопка горной породы
...
### Этап 5: Вертикальная раскопка грунта
...
## 17. Проверка логики исходной задумки

### Что перенесено без изменения

| Элемент | Значение |
| :--- | :--- |
| **Горный инструмент в две руки** | Сохранено. |
| **ЛКМ удары по стене** | Сохранено. |
| **Обломки возле стены от урона** | Сохранено. |
| **Обломки мешают движению и работе** | Сохранено. |
| **Разлет обломков радиусом 1 клетка** | Сохранено. |
| **Уборка обломков лопатой** | Сохранено. |
| **Появление кусков руды из рудной стены** | Сохранено. |
| **Кусок руды не больше 1u** | Сохранено. |
| **Окрас куска руды под тип руды** | Сохранено. |
| **Вертикальная раскопка камня 15 секунд** | Сохранено. |
| **Грунт дает кусок грязи в лопате** | Сохранено. |
| **Вертикальная копка грунта 2 секунды** | Сохранено. |
| **Пол грунта опускается после N действий** | Сохранено. |
| **Центр месторождения** | Сохранено. |
| **Распространение руды за тик с шансом N%** | Сохранено. |
| **Подпорки и балки** | Сохранено. |
| **Базовый шанс обвала 1%** | Сохранено. |
| **Рост риска на 20% за блок** | Сохранено. |
| **Распространение обвала как генерация руды** | Сохранено. |
| **Шанс распространения обвала 80%** | Сохранено. |
| **8 секунд треска дерева** | Сохранено. |
| **30–100 механического урона** | Сохранено. |
| **Формула кислорода** | Сохранена как параметр. |
| **Тихое общение под завалом** | Сохранено. |
| **20% для воздуха / 80% для извлечения** | Сохранено. |
| **Подпорка из 4 бревен по 5 секунд** | Сохранено. |
| **Плавка как следующий этап** | Сохранена как связь с `smithing.md`. |

---

### Что исправлено структурно

1. В исходном Word дважды указан заголовок **«Вертикальная раскопка в горной породе»**. Второй такой блок по содержанию относится к грунту, поэтому он оформлен как **«Вертикальная раскопка грунта»**.
2. Фраза **«пол будет разрушен и смешен на 1 я уровень вниз»** приведена к виду **«пол разрушается, клетка смещается на один уровень вниз»**.
3. Слово **«откропать»** исправлено на **«откопать»**.
4. Значение **N** не заменено числом, потому что в Word оно не указано.
5. Формула кислорода не переписана математически, потому что исходник не задает точный порядок расчета.
6. Раздел плавки не перенесен полностью в шахтерское дело, чтобы не дублировать отдельную цепочку кузнечного дела.
